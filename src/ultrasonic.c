...
/* Busy delay in microseconds using the UsTimer one-shot timer.
 * This is a blocking delay, but the durations we use (2 µs, 10 µs) are tiny.
 */
void Ultra_BusyDelayUs(uint32 microseconds)
{
    if (microseconds == 0u)
    ...
    /* Start hardware one-shot for the requested number of microseconds */
    UsTimer_Start(microseconds);

    /* Spin until notification fires */
    while (UsTimer_HasElapsed() == FALSE)
    {
        /* busy wait – very short delays only */
    }

    /* Clear flag so next delay starts from a clean state */
    UsTimer_ClearFlag();
}
...
static void Ultrasonic_SendTriggerPulse(void)
{
    /* Make sure TRIG is low first */
    (void)Dio_WriteChannel(ULTRA_DIO_TRIG_CHANNEL, STD_LOW);
    ULTRA_DELAY_US(2u);   /* 2 µs low "settling" */

    /* 10 µs high pulse using UsTimer-based delay */
    (void)Dio_WriteChannel(ULTRA_DIO_TRIG_CHANNEL, STD_HIGH);
    ULTRA_DELAY_US(10u);
    (void)Dio_WriteChannel(ULTRA_DIO_TRIG_CHANNEL, STD_LOW);
}
...
void Ultrasonic_Init(void)
{
    ...
    /* Prepare UsTimer flag (Gpt_Init must already be done in DriversInit()) */
    UsTimer_Init();

    /* Defensive: ensure ICU is in NORMAL mode (if API available in your MCAL)
     * Example (commented out because not generated in your current stack):
