...
static inline Icu_ChannelType Icu_ConvertChannelIndexToChannel(Icu_ChannelType ChannelIndex, uint32 u32CoreId);
                                             ...
                                             uint16 NotifyInterval)
{
    ...
    /* ChannelConfig - Address of logic configuration for channel on u32CoreId partition. */
    ChannelConfig = ((*Icu_pCfgPtr[u32CoreId]->Icu_ChannelConfigPtr)[ChannelId]).Icu_IpwChannelConfigPtr;
    ...
    if ((NotifyInterval >= BufferSize) || (NotifyInterval == (uint16)0))
    {
        ...
        if ((uint8) ICU_CIRCULAR_BUFFER == (*(Icu_pCfgPtr[u32CoreId]->Icu_ChannelConfigPtr))[ChannelId].Icu_ChannelProperty)
        {
            global_Mcl_DmaChannelTransferList[10U].Param = MCL_DMA_CH_SET_DESTINATION_SIGNED_LAST_ADDR_ADJ;
            global_Mcl_DmaChannelTransferList[10U].Value = (uint32) (~((uint32)BufferSize * ICU_DMA_NUM_BYTES)) + 1U;
        }
        else
        {
    ...
    Mcl_SetDmaChannelTransferList((*(Icu_pCfgPtr[u32CoreId]->Icu_ChannelConfigPtr))[ChannelId].Mcl_DmaChannel, \
                                   global_Mcl_DmaChannelTransferList, (uint32)11U);

    Mcl_SetDmaChannelCommand((*(Icu_pCfgPtr[u32CoreId]->Icu_ChannelConfigPtr))[ChannelId].Mcl_DmaChannel, \
