...
Use a common ground between all boards. Without common ground the SPI signal
levels are referenced differently and the S32K can read random data.

### Teensy 4.1 To MPU6050/GY-521

The IMU uses I2C on the Teensy. Use the default Teensy 4.1 `Wire` pins:

| MPU6050/GY-521 | Teensy 4.1 | Notes |
|---|---|---|
| `VCC` | `3.3V` | Keep logic at 3.3 V. |
| `GND` | `GND` | Must also connect to S32K ground. |
| `SDA` | pin `18` | Teensy `Wire` SDA. |
| `SCL` | pin `19` | Teensy `Wire` SCL. |
| `AD0` | `GND` | Keeps normal MPU6050 address `0x68`. |
| `INT` | not connected | Polling is enough for the first test. |

Keep the IMU still during startup so the Teensy can measure gyro offset.

### Teensy 4.1 To S32K SPI

The Teensy sends one fixed packet to the S32K. The S32K should be configured as
the SPI slave in S32 Design Studio.

| Teensy 4.1 | S32K LPSPI slave | Notes |
|---|---|---|
| pin `10` CS | PCS / chip select input | One packet per CS transaction is easiest. |
| pin `11` MOSI | SIN / MOSI input | Teensy data to S32K. |
| pin `12` MISO | SOUT / MISO output | Not used yet, but wire it for future commands. |
| pin `13` SCK | SCK input | Clock generated by Teensy. |
| `GND` | `GND` | Required. |

Current packet direction is only:

```text
Teensy -> S32K
```

The S32K does not send command packets back yet.

### Camera And Missing Component Status

The camera can stay on the S32K while the Teensy handles the IMU.

For the current setup the Teensy packet should report:
...
- Camera 1 is handled by the S32K.
- Camera 2 is not connected.
- If camera 2 later moves to Teensy, set `camera2Status = TEN`.
- If a sensor is missing, keep sending packets but clear the related valid flag.

## How To Load The S32K Board

Use S32 Design Studio for the S32K project.

1. Open S32 Design Studio.
2. Open workspace:

```text
C:\Users\Navif\workspaceS32DS.3.6.3
```
...
7. Build the `Debug_FLASH` configuration.
8. Flash/debug using the existing S32K launch configuration.
9. Reset the S32K after flashing.

If the Teensy is not connected yet, the display should show `SPI: WAIT`.
Press `SW2` to enable demo packets and prove the S32K display path works.

## How To Load The Teensy Board

Use PlatformIO only for the Teensy firmware project.

1. Connect the Teensy 4.1 by USB.
2. Open PowerShell.
3. Go to the Teensy project:

```powershell
cd "C:\Users\Navif\Downloads\Fontys\Year2\Expo1&2\NXP\Teensy+IMU,Test"
```
...
6. Open serial monitor if you want to check values on the laptop:

```powershell
pio device monitor
```

Expected Teensy startup checks:

- MPU6050/GY-521 detected at I2C address `0x68`.
- Gyro calibration finishes while the IMU is still.
- Roll and pitch change when the IMU is tilted.
- Yaw rate changes when the IMU is rotated.
- SD status is `ready` only when the SD card is present and logging works.

## Power-Up Order For Full Test

Use this order for the cleanest first test:

1. Wire IMU to Teensy.
2. Wire Teensy SPI to S32K.
3. Connect common ground.
4. Flash Teensy firmware.
5. Flash S32K firmware.
6. Power Teensy and keep IMU still during calibration.
7. Power or reset S32K.
8. Watch the S32K display.
...
Then move the IMU. `R10`, `P10`, `Y10`, and `GZ` should change.
