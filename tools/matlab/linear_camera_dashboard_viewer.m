function linear_camera_dashboard_viewer(portName)
% Live viewer for RUNTIME_TEST_LINEAR_CAMERA UART telemetry.
% Dashboard layout: raw+filtered, gradient, and decoded stats.
% Example:
%   linear_camera_dashboard_viewer("COM6")
...
grid(axSignal, "on");
hold(axSignal, "on");
xlim(axSignal, [0 sampleCount - 1]);
ylim(axSignal, [0 255]);
xlabel(axSignal, "Pixel");
ylabel(axSignal, "Intensity (0..255)");
title(axSignal, "Raw + Filtered");

hRaw = plot(axSignal, 0:sampleCount-1, zeros(1, sampleCount), "LineWidth", 1.2, ...
            "Color", [0.00 0.35 0.80], "LineStyle", "-", "DisplayName", "Raw");
hFiltered = plot(axSignal, 0:sampleCount-1, zeros(1, sampleCount), "LineWidth", 1.2, ...
                 "Color", [0.90 0.45 0.10], "LineStyle", "-", "DisplayName", "Filtered");
hLeft = xline(axSignal, 0, "-", "L", "Color", [0.85 0.20 0.20], "Visible", "off");
hRight = xline(axSignal, 0, "-", "R", "Color", [0.20 0.60 0.20], "Visible", "off");
hFinishLeft = xline(axSignal, 0, ":", "FgL", "Color", [0.55 0.10 0.55], "Visible", "off");
hFinishRight = xline(axSignal, 0, ":", "FgR", "Color", [0.55 0.10 0.55], "Visible", "off");
hideFromLegend(hLeft);
hideFromLegend(hRight);
hideFromLegend(hFinishLeft);
hideFromLegend(hFinishRight);
legend(axSignal, [hRaw, hFiltered], {"Raw", "Filtered"}, "Location", "northwest");

axGrad = nexttile(t, 3);
grid(axGrad, "on");
hold(axGrad, "on");
xlim(axGrad, [0 sampleCount - 1]);
ylim(axGrad, [-130 130]);
xlabel(axGrad, "Pixel");
ylabel(axGrad, "Gradient (-127..127)");
title(axGrad, "Gradient + Thresholds");

hGrad = plot(axGrad, 0:sampleCount-1, zeros(1, sampleCount), "LineWidth", 1.1, ...
             "Color", [0.10 0.55 0.30], "DisplayName", "Gradient");
hWeakPos = yline(axGrad, 0, "--", "Weak+", "Color", [0.75 0.65 0.10]);
hWeakNeg = yline(axGrad, 0, "--", "Weak-", "Color", [0.75 0.65 0.10]);
hStrongPos = yline(axGrad, 0, "-", "Strong+", "Color", [0.80 0.15 0.15]);
hStrongNeg = yline(axGrad, 0, "-", "Strong-", "Color", [0.80 0.15 0.15]);
hEdgesPos = scatter(axGrad, nan, nan, 28, [0.85 0.20 0.20], "filled", "DisplayName", "Rising edges");
hEdgesNeg = scatter(axGrad, nan, nan, 28, [0.20 0.20 0.80], "filled", "DisplayName", "Falling edges");
hideFromLegend(hWeakPos);
hideFromLegend(hWeakNeg);
hideFromLegend(hStrongPos);
hideFromLegend(hStrongNeg);
legend(axGrad, [hGrad, hEdgesPos, hEdgesNeg], ...
       {"Gradient", "Rising edges", "Falling edges"}, ...
       "Location", "northwest");

axStats = nexttile(t, 2, [2 1]);
axis(axStats, "off");
title(axStats, "Decoded Debug State");
hText = text(axStats, 0.0, 1.0, "", "Units", "normalized", ...
...
while isvalid(fig)
    ...
    if ~isempty(latestFrame) && ~fig.UserData.isFrozen
        ...
        title(axSignal, sprintf("Vision Debug Data | frame %u", latestFrame.seq));
        title(axGrad, sprintf("Gradient | maxAbs=%u", latestFrame.maxAbsGradient));

        hText.String = buildStatsText(latestFrame);
        drawnow limitrate;
    end
end
end
...
function textOut = buildStatsText(frame)
textOut = sprintf([ ...
    'Steering\n' ...
    'status:      %s\n' ...
    'confidence:  %3d\n' ...
