...
- `src/app/modes/bench_teensy_link.c`: S32K-master Teensy SPI link bring-up mode and runtime test menu item.
...
- The `APP_TEST_NXP_CUP_TESTS` menu contains the individual test screens: `Camera`, `ESC`, `Servo`, `Ultrasonic`, `Cam+Servo`, `Simple test drv`, `Serial tune`, `Ultra+ESC`, `Teensy Link`, and `Receiver - x`.
