...
The Teensy reads the physical MPU6050 at 100 Hz, filters its orientation, and
sends the existing IMU fields in the current 84-byte Teensy-S32K SPI frame.
The packet format and frame size did not change.
...
The normal acquisition/service call is in
`firmware/teensy/src/services/teensy_race_runtime.cpp`.
...
The driver probes both `0x68` and `0x69`. The optional Teensy display path uses
`Wire1` on pins 17/16, so it does not share the IMU bus.
...
SD logging is not part of this first IMU-only integration. The SPI telemetry
and serial status output are the validation paths for now.
...
- Camera 0 continues to report the current camera-vision result. Camera 1
  remains missing/stale until a second camera pipeline exists.
