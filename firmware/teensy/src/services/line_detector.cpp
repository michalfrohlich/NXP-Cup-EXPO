...
{
    return scaleToPct(edgeStrength,
                      VISION_LINEAR_CONF_EDGE_LOW,
                      VISION_LINEAR_CONF_EDGE_HIGH);
}

static uint8_t blendConfidence(uint8_t edgeStrengthConfidence,
                               uint8_t contrastConfidence)
{
...
static uint8_t clampSplitPoint(float center)
{
    ...
    uint8_t maxSplit =
        (uint8_t)(VISION_LINEAR_BUFFER_SIZE - 1U - VISION_LINEAR_SPLIT_MARGIN_PX);
...
static void filterSignal(const uint16_t *pixels, uint16_t *filtered)
{
    filtered[0] = pixels[0];
    filtered[1] =
        (uint16_t)(((uint32_t)pixels[0] + ((uint32_t)pixels[1] * 2U) +
                    (uint32_t)pixels[2]) /
                   4U);
    ...
    for (uint16_t i = 2U; i < (VISION_LINEAR_BUFFER_SIZE - 2U); i++)
    {
        uint32_t value =
            ...
            ((uint32_t)pixels[i - 1U] * 4U) +
            ((uint32_t)pixels[i] * 6U) +
            ((uint32_t)pixels[i + 1U] * 4U) +
            (uint32_t)pixels[i + 2U];
    ...
    filtered[VISION_LINEAR_BUFFER_SIZE - 2U] =
        (uint16_t)(((uint32_t)pixels[VISION_LINEAR_BUFFER_SIZE - 3U] +
                   ...
                   4U);
    filtered[VISION_LINEAR_BUFFER_SIZE - 1U] =
        pixels[VISION_LINEAR_BUFFER_SIZE - 1U];
}

static void computeGradient(const uint16_t *filtered,
                            int16_t *gradient,
                            uint16_t &maxAbsGradient)
{
    ...
    {
        int32_t value =
            -((int32_t)filtered[i - 2U]) -
            ((int32_t)filtered[i - 1U] * 2) +
            ((int32_t)filtered[i + 1U] * 2) +
            (int32_t)filtered[i + 2U];
...
{
    uint16_t minWidth =
        ((uint16_t)VISION_LINEAR_NOMINAL_LANE_WIDTH *
...
void LineDetector::processDebug(const uint16_t *pixels,
                                VisionOutput &out,
...
void LineDetector::processImpl(const uint16_t *pixels,
                               VisionOutput &out,
                               LineDetectorDebugOut *debug)
{
    ...
    if ((debug != nullptr) &&
        ((debug->mask & VLIN_DBG_FILTERED) != 0U) &&
    ...
    if ((debug != nullptr) &&
        ((debug->mask & VLIN_DBG_GRADIENT) != 0U) &&
    ...
    edgeHighThreshold =
        (uint16_t)(((uint32_t)maxAbsGradient * VISION_LINEAR_EDGE_HIGH_PCT) / 100U);
    ...
    splitPoint = (isLocked_ != 0U) ? clampSplitPoint(lastTrackCenter_)
    ...
    for (uint16_t i = 2U; i < (VISION_LINEAR_BUFFER_SIZE - 2U); i++)
    {
        ...
        if ((strength >= edgeLowThreshold) && (isLocalMaximum(gradient, i) != 0U))
        {
            ...
            else
            {
                ...
                if ((leftStrength >= edgeHighThreshold) ||
                    (rightStrength >= edgeHighThreshold))
                {
                    isCandidate = 1U;
                }
            }
        ...
        for (uint8_t li = 0U; li < leftCandidateCount; li++)
        {
            for (uint8_t ri = 0U; ri < rightCandidateCount; ri++)
            {
                ...
                if (rightIdx > leftIdx)
                {
                    ...
                    {
                        uint16_t widthErr =
                            absDiffU8(width, VISION_LINEAR_NOMINAL_LANE_WIDTH);
                        uint16_t centerErr = absDiffU8(center, splitPoint);
                        uint16_t strengthSum =
                            (uint16_t)(leftCandidateStrengths[li] +
                                       rightCandidateStrengths[ri]);
                        int32_t score =
                            (int32_t)((uint32_t)widthErr * 8UL) +
                            (int32_t)centerErr -
        ...
        (bestRightIdx > bestLeftIdx))
    {
        ...
        expectedFinishGap =
            (uint8_t)(((uint16_t)laneWidth * VISION_FINISH_CENTER_GAP_MM) /
        ...
        for (uint8_t e = 0U; e + 1U < edgeCandidateCount; e++)
        {
                ...
                (rightGapEdge > leftGapEdge))
            {
                ...
                if ((gap > 0U) &&
                    ...
                    (widthMatchesFinish(gap, expectedFinishGap) != 0U) &&
