...
bool Mpu6050Imu::sample(uint32_t nowUs)
{
    ...
    dtUs = filterStarted_ ? (uint32_t)(nowUs - lastSampleUs_) : TEENSY_LINK_SENSOR_INTERVAL_US;
    if ((dtUs == 0U) || (dtUs > 100000U))
    {
        dtUs = TEENSY_LINK_SENSOR_INTERVAL_US;
    }
...
const TeensyLinkImuSample &Mpu6050Imu::latest() const
{
    return latest_;
}
...
bool Mpu6050Imu::configure()
{
    ...
    return writeRegister(MPU6050_REG_PWR_MGMT_1, 0x01U) &&
           writeRegister(MPU6050_REG_SMPLRT_DIV, 9U) &&
           writeRegister(MPU6050_REG_CONFIG, 3U) &&
...
bool Mpu6050Imu::calibrateGyro()
{
    ...
    for (uint16_t i = 0U; i < MPU6050_CALIBRATION_SAMPLES; i++)
    {
        ...
        if ((fabsf(gx) > MPU6050_CALIBRATION_MAX_GYRO_DPS) ||
            ...
            (accelNorm < MPU6050_ACCEL_TRUST_MIN_G) ||
            (accelNorm > MPU6050_ACCEL_TRUST_MAX_G))
        {
            return false;
        }
